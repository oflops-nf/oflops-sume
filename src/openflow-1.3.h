...
#define OXM_HEADER(OXM_VENDOR, OXM_CLASS, OXM_FIELD, OXM_HASMASK, OXM_LENGTH) \
    (((uint64_t) (OXM_CLASS) << 48) |                               \
     ((uint64_t) (OXM_FIELD) << 41) |                               \
     ((uint64_t) (OXM_HASMASK) << 40) |                             \
     ((uint64_t) (OXM_LENGTH) << 32) |                              \
     (OXM_VENDOR))

#define OXM_HEADER_FMT "%#"PRIx32":%d:%d:%d:%d"
...
struct ofp_flow_mod {
    ...
    int8_t instructions[0]; /* Instruction set */
};
...
struct ofp_table_features {
    ...
    // struct ofp_table_feature_prop_type properties[0];
};
