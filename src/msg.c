...
int
make_ofp_barrier_req(void **bufferp) {
  struct ofp_hello *p;
  *bufferp = xmalloc(sizeof(struct ofp_hello));
  p = *(struct ofp_hello **)bufferp;
  ofp_init(&p->header, OFPT_BARRIER_REQUEST, sizeof(struct ofp_hello));
  return sizeof(struct ofp_hello);
}
...
make_ofp_flow_modify_output_port(void **buferp, struct flow *fl, uint32_t out_port,
  ...
  *buferp = xmalloc(len);
  if(make_flow_mod(*buferp, OFPFC_MODIFY, len, fl) < 0 )
    fail("Error: failed to create flow modification packet.");
  struct ofp_flow_mod *ofm = *buferp;
  p = (struct ofp_action_output *)ofm->actions;
  ofm->idle_timeout = htons(idle_timeout);
  ofm->hard_timeout = htons(OFP_FLOW_PERMANENT);
