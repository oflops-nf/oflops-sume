...
cap_filter_to_pcap(cap_filter f, char *buf, int len) {
	...
	char ret[20];
	if(f.proto) 
		c += snprintf(buf + c, len, "%s ", f.proto);
	if (f.src) 
...
setup_channel(oflops_context *ctx, test_module *mod, enum oflops_channel_name ch)
{
    ...
    if((ch == OFLOPS_CONTROL) || (ch_info->cap_type == PCAP)) {
        ...
        if(pcap_compile(ch_info->pcap_handle, &filter, buf, 1, net) == -1) {
            pcap_perror(ch_info->pcap_handle, "PCAP_COMPILE:");
            exit(1);
        }
    ...
    //else open the nf packet gen capturer
        ...
        if ((ch_info->nf_cap = nf_cap_enable(ch_info->dev, ctx->snaplen)) == NULL) {
            fprintf(stderr, "ENABLING FAILED\n.");
            exit(1);
        }
        if (ch_info->rx_measurement != 0) {
            if (nf_enable_rx_measurement(ch_info->dev) < 0) {
                exit(1);
            }
        }
    ...
    for (ix = 0; ix < count; ix++) {
        ...
        nf_cap_add_rule(filter_count++, proto_to_int(f[ix].proto),
                f[ix].src, f[ix].dst, f[ix].port,
                f[ix].proto_mask, f[ix].src_mask, f[ix].dst_mask,
                f[ix].port_mask);
        usleep(5);
		}
    printf("NetFPGA capture on %s (sock:%d)\n", ch_info->dev , ch_info->pcap_fd );
  } else {
