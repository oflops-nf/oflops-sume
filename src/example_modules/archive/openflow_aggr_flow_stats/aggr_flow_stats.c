...
#define LOG_FILE "of_aggr_flow_stats.log"
 ...
 *   the module will print on a file called "of_aggr_flow_stats.log" for each capture packet a
...
int destroy(struct oflops_context *ctx) {
  ...
  for (i = 0; i < trans_id; i++) {
    ...
    ix[0]++;
    snprintf(msg, 1024, "stats:id:%u:snd:%ld.%06ld rcv:%ld.%06ld:diff:%u",i,  
...
int handle_timer_event(oflops_context * ctx, struct timer_event *te)
{
    ...
    if(strcmp(str, GETSTAT) == 0) {
        ...
        struct ofp_aggregate_stats_request *reqp = (struct ofp_aggregate_stats_request *)
            (b + sizeof(struct ofp_multipart_request));
        ...
        flow.nw_dst = htonl(flow_netmask);
        match_t match;
        match.flow = flow;
        match.wc = mask;
        match.wildcarded = wildcarded;
        match_to_payload(&match, (void**)(&reqp->match.oxm_fields));
...
of_event_other(oflops_context *ctx, const struct ofp_header * ofph) {
    ...
    if(ofph->type == OFPT_MULTIPART_REPLY) {
        ...
        if(ntohs(ofpr->type) == OFPMP_AGGREGATE) {
            sprintf(msg, "%d", ntohl(ofph->xid));
            gettimeofday(&now, NULL);
            oflops_log(now, OFPT_STATS_REPLY_FLOW, msg);
            if((ntohs(ofpr->flags) & OFPMPF_REPLY_MORE) == 0) {
