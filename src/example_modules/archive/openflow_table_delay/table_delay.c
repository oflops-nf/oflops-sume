...
start(oflops_context * ctx) {
	...
	fl.mask = OFPFW_IN_PORT | OFPFW_DL_SRC  
		| OFPFW_DL_TYPE | OFPFW_NW_PROTO | OFPFW_NW_TOS  
		| (32 << OFPFW_NW_DST_SHIFT) | (32 << OFPFW_NW_SRC_SHIFT) 
	...
	for (counter = 0; counter < flows; counter++) {	
	   ...
	   make_ofp_flow_add(&b, &fl, ctx->channels[OFLOPS_DATA2].of_port, -1, 60);
