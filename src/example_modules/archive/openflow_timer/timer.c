 ...
 *  extended per flow measurement information. The information is printed in log "of\_timer.log"
 *  file.
 *
...
start(oflops_context * ctx) {
    ...
    make_ofp_flow_add(&b, fl, mask, ctx->channels[OFLOPS_DATA2].of_port, 1, 0, 120);
...
destroy(oflops_context *ctx) {
    ...
    for(i = 0; i< flows; i++) {
        ...
        if(first[i].tv_sec > 0) {
            snprintf(data, 1024, "flow_timeout:%s:%ld.%06ld:%ld.%06ld:%d:%f", inet_ntoa(addr),
...
int handle_timer_event(oflops_context * ctx, struct timer_event *te) {
    char *str = te->arg;
    int i;
    flow_t *fl = (struct flow*)xmalloc(sizeof(flow_t));
    flow_t *mask = xmalloc(sizeof(flow_t));
    void *b; //somewhere to store message data
    int len;
    ...
    if (strcmp(str, BYESTR) == 0) {
        ...
        for(i = 0; i < flows; i++) {
            len = make_ofp_flow_add(&b, fl, mask, OFPP_IN_PORT, 1, 0, 10);
...
of_event_flow_removed(oflops_context *ctx, const struct ofp_flow_removed * ofph) {
    ...
    match_t match; 
    tlv_chain_deserialize(ofph->match.oxm_fields, &tlv_chain, ofph->match.length);
    payload_to_match(&match, &tlv_chain);

    int id = ntohl(match.flow.nw_dst) - ntohl(inet_addr(network));
    if((id < 0) || (id >= flows)) {
        addr.s_addr = match.flow.nw_dst;
        printf("Invalid ip %s\n", inet_ntoa(addr));
        return 0;
    }
