 ...
 * concurent ionserted flow. The measurement includes both the delay to generate 
 * the packet_out event and the delay to install the flow
 *
 * Parameters:
 *
...
//logging filename
#define LOG_FILE "of_reactive.log"
 ...
 * Probe packet size
 */
uint32_t pkt_size = MAX_PKT_SIZE;
...
int destroy(oflops_context *ctx) {
	...
	for (i = 0; i < flows; i++) {
		...
		snprintf(msg, 1024, "%d:%ld.%06ld:%ld.%06ld:%ld.%06ld", i,
...
of_event_packet_in(oflops_context *ctx, const struct ofp_packet_in * pkt_in) {
    ...
    match_t match;
    tlv_chain_deserialize(pkt_in->match.oxm_fields, &tlv_chain, pkt_in->match.length);
    payload_to_match(&match, &tlv_chain);
	switch(pkt_in->reason) {
		case  OFPR_NO_MATCH:
			pktgen = extract_pktgen_pkt(ctx, ntohs(match.wc.in_port), (void *)pkt_in->data,
